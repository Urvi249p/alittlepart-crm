        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr style={{ backgroundColor: colors.headerBg }} className="text-white">
              <Th>Deadline</Th><Th>Client</Th><Th className="border-l border-[#F5E4D7]">Product &amp; Details</Th>
              <Th align="right" className="border-l border-[#F5E4D7]">Selling ₹</Th><Th align="right">Cost ₹</Th><Th align="right">Share ₹</Th><Th align="right">Advance</Th><Th align="right">Balance</Th>
              <Th>Delivery Place</Th><Th>Occasion</Th><Th>Source</Th><Th>Packaging</Th><Th className="border-l border-[#F5E4D7]">Status</Th><Th>Notes</Th><Th>Actions</Th>
            </tr>
          </thead>
          <tbody>
            {filteredOrders.map(order => {
              ...
              return (
                <tr key={order.id} style={rowStyle} className="border-b hover:bg-opacity-80 transition">
                  ...
                  <Td className="border-l border-[#F5E4D7]"><div className="flex min-w-[150px] flex-col items-start gap-1"><span className="px-2 py-0.5 rounded-lg text-xs" style={{ backgroundColor: colors.coralPale, color: colors.coralDark }}>{order.productType}</span>{getProductDetailsText(order) && <span className="text-xs" style={{ color: colors.textLight }}>{getProductDetailsText(order)}</span>}</div></Td>
                  <Td align="right" className="border-l border-[#F5E4D7]"><span style={{ color: colors.text }}>{order.sellingPrice ? fmtMoney(order.sellingPrice) : '-'}</span></Td>
                  ...
                  <Td align="right"><span className={order.status === 'Completed' ? 'font-semibold text-green-700 inline-flex items-center gap-1' : balance > 0 ? 'font-semibold text-red-600' : 'text-gray-500'}>{order.status === 'Completed' && <CheckCircle2 className="w-4 h-4" aria-hidden="true" />}{fmtMoney(balance)}</span></Td>
                  <Td><span className={order.deliveryPlace && order.deliveryPlace.trim().toLowerCase() !== 'surat' ? 'font-bold' : ''}>{order.deliveryPlace || '-'}</span></Td>
          ...
          <tfoot>
            <tr className="font-semibold" style={{ backgroundColor: colors.coralPale, color: colors.text }}>
              <Td colSpan={3}>TOTAL ({filteredOrders.length})</Td>
              <Td align="right" className="border-l border-[#F5E4D7]">{fmtMoney(filteredOrders.reduce((s, o) => s + (parseFloat(o.sellingPrice) || 0), 0))}</Td>
...
function Th({ children, align = 'left', className = '' }) {
  return <th className={`px-3 py-2.5 text-${align} text-xs font-semibold uppercase tracking-wide whitespace-nowrap ${className}`}>{children}</th>;
}

function Td({ children, align = 'left', colSpan, className = '' }) {
  return <td colSpan={colSpan} className={`px-3 py-3 text-${align} whitespace-nowrap ${className}`}>{children}</td>;
}
